...
function gameOver(){
  ...
  const gameOverMessage = `Juego terminado! Puntuación Final - <span class = 'badge'>${score}</span> Click en botón 'Iniciar Juego'`;
...
function chooseCard(card) {
  if(canChooseCard()) {
    ...
    setTimeout(() => {
      flipCards(false);
      updateStatusElement(currentGameStatusElem,"block", primaryColor,"Posiciones de cartas reveladas!");
      endRound();
    }, 3000);
    cardsRevealed = true;
  }
...
function outputChoiceFeedBack(hit) {
  if(hit) {
    updateStatusElement(currentGameStatusElem, "block", winColor, "Atinaste!! - Muy Bien!! :)");
  }
  else {
    updateStatusElement(currentGameStatusElem, "block", loseColor, "Perdiste!! :(");
  }
}
...
function initializeNewGame() {
  ...
  updateStatusElement(scoreElem,"block",primaryColor,`Puntos <span class='badge'>${score}</span>`);
...
function shuffleCards() {
  ...
  function shuffle() {
    ...
    if(shuffleCount == 500) {
      ...
      dealCards();
      updateStatusElement(currentGameStatusElem, "block", primaryColor, "Haz click en la carta que crees que es el as de picas...");
    }
    else {
      shuffleCount++
    }
